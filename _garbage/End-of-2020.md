...
我自己的业余时间乱搞的东西的话，大概有这些：
 ...
 - **kimageformats**:
   我的看图工具的图像格式支持靠的就是 kimageformats 这个来自 KDE 的图像格式插件，然后在我的使用过程中，陆续发现了一些 PSD 文件无法被正常显示，于是在有一天再次发现一张新的 PSD 文件无法正常显示时，决定研究一下原因。挂上调试后发现 kimageformats 的 PSD 支持并不完整，而我遇到问题的大都是 16bit/channel 的文件，于是后来（在 chris 的帮助下）加了支持，算是解决了这个问题。
   然而其实支持还是不完整，灰度图以及 CMYK 格式之类的还是不支持，但感觉至少目前已经覆盖了我机器上几乎所有的 PSD 文件，于是我也暂时不打算去加别的支持了。
 - **QMidiPlayer**, **MSYS2** 和 **KDE Craft**:
 ...
 - **KTextEdit**:
   其实是 Kate 的问题，因为 Notepad++ 在很早之前不知道哪个版本 break 掉了暗色主题的 Markdown 显示支持（会显示成亮瞎眼的白色），于是考虑切换到了 Kate，但动态文字换行的需求一直觉得不能从工具栏点比较蛋疼，于是找了一下，发现是我眼挫漏看了，确实是可以配置让工具栏显示动态文字换行的。后来觉得如果可选列表里如果这个选项有图标的话大概会好找很多，于是研究了下怎么加图标，然后丢了个 patch 上去。
 ...
 - **gitea**:
   我离职后到新公司后，因为要做的事情变得更杂了，于是接触新东西的情况变得更频繁，也就需要一个地方来记笔记，就选了 Gitea 的 Wiki 功能来做这件事。用稍久后突然有了想传图的需求，就开始搜有没有人做过这个功能，有的话是什么完成度。后来发现了一个人做了一般弃了，而我那段时间偏偏就是很想插图片，于是打算上手改改试试。
   不过后来发现那个人的代码和 master HEAD 差太远，没法直接用了，于是还是相当于我自己重搞（尽管算是大致给了个方向应该从哪儿改起），然后改的过程发现涉及界面相关的问题分开提了 PR，结果界面相关的 PR 合了，插图的功能卡到了现在（
   插图的功能做倒是做完了，也能用，但发现用的时候会报错说一个文件被占用删不掉。我盯了大概半天没看出来问题在哪，于是在 PR 请求协助，最后似乎也没人给出帮助，然后随着我不在有插图的打算了，就搁置了这个功能 PR。
 ...
 - **blender**:
   其实还是蛮想建议更多人尝试使用自由软件的，于是尽管有时候自己也不是很会用的时候，也会在看到别人有意愿尝试后试着解答一些别人遇到的问题。码喵在尝试使用 blender 后遇到了设为启动模板功能失效的问题，我本地则用的是我自己的启动模板，于是试着帮码喵看了一下，发现把我自己的模板文件丢进去是管用的，然后发现似乎是如果之前没进行过设置就直接存为启动模板就会失败。尝试过程发现这个问题竟然存在了好几个版本而且似乎没人报，于是开了 bug ，也决定拉代码定位一下。
...
工作时候遇到的就不是很多了...
 ...
 - **Qt**:
   这回只是报的 bug。也是因为我的看图，因为开发环境用到了 WSL，我用了 WSL2，后来发现访问 WSL2 目录下的图片时看图没法正常载入，以为是看图的 bug，于是跟着定位了一下，最后发现问题在 Qt 和巨硬。巨硬 WSL2 网络位置故意用了个 `\\wsl$\` URL，然后 URL 里其实不能出现 `$` 字符，于是 Qt 不能正常处理这个 URL，不过有意思的是 GIMP 倒是能应付的好好的，于是给 Qt 报了这个 bug...
   因为这个 bug 还是蛮显而易见的，于是真要修也好修，但给 Qt 扔一次 patch 还蛮麻烦的，又要写的漂亮又得有测试，而且准备一次 Qt 自身的开发环境还蛮麻烦的，于是我自己就没兴趣修这个 bug 了，就把这问题告诉了竹子看看有没有兴趣修，然后我自己给看图里绕了一下这个 bug，就没再管了。后来竹子似乎也没再理会这个 bug，最后还是官方去修了这个 BUG，5.15.2 修了，于是后来把看图版本也改到了 5.15.2，WSL2 图片载入的问题也算是改彻底了。
...
既然上面的段落提到了“软件质量和软件本身是专有软件还是开源软件没必然联系”的结论，就顺带提一下一些别的看法。今年内我的一些尝试其实涉及了不少乱七八糟的软件，也留下了一些不同的印象：
 ...
 - **FL Studio**（专有）
   上面提到了我的两次 bug 反馈都没得到我觉得理想的答复，他们倒是照常推了一些更新。在我反馈 bug 之后顺带看了下论坛，发现 [ImageLine DMCA 了一个用来改 FL 外观的开源项目](https://github.com/github/dmca/blob/master/2020/09/2020-09-14-ImageLine.md)。
   我喜欢自由软件的众多原因之一是，如果软件出了什么问题，你官方还不想自己解决，那我可以自己光明正大的修改它甚至再发布来使其符合我自己的需求，尽管并非所有专有软件都反对社区做这种事，但 IL 因为这么点事儿就发 DMCA 确实是挺让我觉得反感的。
 - **Godot**（自由，有公司实体和全职开发者，社区主导开发）
   其实 Godot Engine 关注那么久，真正使用之前看上去“Sounds promising”，实际用起来还是有看到一些或大或小的问题。比如 GridMap 对于 Grid 方向的存储方式是个压根没文档也没什么规律的枚举，然后文档写着个“看代码去”，TileMap 功能编辑 TileSet 的地方也有一些虽然不影响功能但影响体验的小毛病。
 ...
 - **KDE**（自由，有公司实体，社区主导开发）
   在试着改 KDE 的一些代码的时候，观察到 KDE 的代码质量总体还是很高的，尽管项目的完成度不一定高。KDE for Windows 相关的问题看上去很少有人关注，但作为社区驱动的项目，KDE for Windows 用户量和维护者本身也不多，于是相关问题反馈处理没有 Linux 环境下 KDE 问题的反馈处理及时也算是可以理解的吧。
